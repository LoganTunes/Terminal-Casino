...
# ----------------------------------------------------------------
# Window / lobby setup (Adjusted to 1280x720 to fix screen clipping)
# ----------------------------------------------------------------

LOBBY_WIDTH, LOBBY_HEIGHT = 1280, 720
...
# ----------------------------------------------------------------
# Lobby layout: Scaled for 1280x720 display
# ----------------------------------------------------------------

COLS, ROWS = 4, 2
CARD_W, CARD_H = 260, 200
GRID_MARGIN_X, GRID_TOP = 60, 180
GAP_X, GAP_Y = 35, 30
...
def draw_card_icon(surface, rect, index, accent):
    cx, cy = rect.centerx, rect.top + 65
    if index == 0:
...
def draw_lobby(balance, hover_idx):
    ...
    screen.blit(title_surf, (LOBBY_WIDTH // 2 - title_surf.get_width() // 2, 50))
    sub_surf = subtitle_font.render(
        "Choose a table to play - your bankroll follows you everywhere",
        True,
        CREAM_WHITE,
    )
    screen.blit(sub_surf, (LOBBY_WIDTH // 2 - sub_surf.get_width() // 2, 110))
...
# CRITICAL WEBASSEMBLY/DESKTOP ENTRYPOINT FIX:
asyncio.run(main())
